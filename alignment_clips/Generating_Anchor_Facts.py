...
log_file = sent_dir + "/Anchor_Facts.log"
    ...
    for i in range(len(starting)):
        if str(starting[i]) != str("0"):
            if "(" not in starting[i] and ")" not in starting[i]:
                print(
                    "(anchor_type-english_id-hindi_id anchor "
                    + str(i + 1)
                    + " "
                    + starting[i]
                    + ")"
    ...
    for i in range(len(potential)):
        if str(potential[i]) != str("0"):
            if "(" not in potential[i] and ")" not in potential[i]:
                hids =potential[i].split("/")
                for h in hids:
                    print(
                        "(anchor_type-english_id-hindi_id potential "
                        + str(i + 1)
                        + " "
                        + h
                        + ")\n"
    ...
    for i in range(len(probable)):
        if str(probable[i]) != str("0"):
            if "(" not in probable[i] and ")" not in probable[i]:
                ...
                for h in hids:
                    print(
                        "(anchor_type-english_id-hindi_id probable "
                        + str(i + 1)
                        + " "
                        + h
                        + ")\n"
